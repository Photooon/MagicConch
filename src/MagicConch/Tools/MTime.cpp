...
MTime MTime::now()
{
	...
	MTime now(localTime->tm_year + 1900, localTime->tm_mon + 1, localTime->tm_mday, localTime->tm_hour, localTime->tm_min);	
																			//精确到分，有点bug，矫正一下
	return now;
}
...
{
	if (this->year == b.year && this->month == b.month && this->day == b.day && this->hour == b.hour && this->minute == b.minute)
	{
		return true;
	}
	else
